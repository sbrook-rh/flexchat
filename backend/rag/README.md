...
- **Pre-computed embeddings storage**: Documents must include embeddings (generated by Node backend)
...
- **Query embeddings**: Generates embeddings for query operations only

### Architecture

The wrapper service handles:
- **Document storage**: Validates and stores documents with pre-computed embeddings from Node.js backend
- **Query embeddings**: Generates embeddings for incoming queries using configured provider
...
**Notes**:
- Wrapper stores embedding metadata exactly as provided by Node backend
- Top-level `embedding_provider` and `embedding_model` fields are stored without validation
